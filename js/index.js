...
function loadLinks(profileLinks){
	...
	while(i<profileLinks.length){
		...
		for(j=i;j<profileLinks.length&&j<i+4;j++){
...
function loadResume(resumedownload){
	...
	while(i<resumedownload.length){
		...
		for(j=i;j<resumedownload.length&&j<i+5;j++){
			resumedownloadInnerHTML+='<div class="col s2">													<a href="'+resumedownload[j].link+'" target="_blank" >					<img src="img/'+resumedownload[j].icon+'" alt="'+resumedownload[j].name+'">	Resume	</a></div>';
		}
